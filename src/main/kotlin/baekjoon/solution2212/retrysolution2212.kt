...
각 집중국은 센서의 수신 가능 영역을 조절할 수 있다. 집중국의 수신 가능 영역은 고속도로 상에서
연결된 구간으로 나타나게 된다. N개의 센서가 적어도 하나의 집중국과는 통신이 가능해야 하며,
...
fun main() = with(BufferedReader(InputStreamReader(System.`in`))) {
    ...
    val k = readLine().toInt() // 집중
    var result = 0 // 결과
    var sensorBetweenLength = arrayListOf<Int>()

    var coordinatesArr = Array<Int>(n) { 0 }
    readLine().split(" ")
        .mapIndexed { index, coordinate ->
            coordinatesArr[index] = coordinate.toInt()
        }
    coordinatesArr.sort()

    for (i in 0 until coordinatesArr.size - 1) {
        sensorBetweenLength.add(coordinatesArr[i + 1] - coordinatesArr[i])
    }

    sensorBetweenLength.sortByDescending { it }
    val resultList = sensorBetweenLength.drop(k - 1)
    resultList.forEach {
        result += it
    }
    println(result)
